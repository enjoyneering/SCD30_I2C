...
bool SCD30_I2C::begin(uint32_t speed, uint32_t stretch)
{
  ...
  Wire.setClock(speed);                      //experimental! AVR I2C bus speed 31kHz..400kHz, library default 100000Hz

  Wire.setWireTimeout(stretch, false);       //experimental! library default 25000usec, true=Wire hardware will be automatically reset on timeout
...
bool SCD30_I2C::begin(uint8_t sda, uint8_t scl, uint32_t speed, uint32_t stretch)
{
  ...
  Wire.setClock(speed);                      //experimental! ESP8266 I2C bus speed 1kHz..400kHz, library default 100000Hz

  #if defined (ESP8266)
  Wire.setClockStretchLimit(stretch);        //experimental! library default 150000usec
  #else
  Wire.setTimeout(stretch / 1000);           //experimental! library default 50msec
  #endif

#elif defined (_VARIANT_ARDUINO_STM32_)
bool SCD30_I2C::begin(uint32_t sda, uint32_t scl, uint32_t speed)
{
  Wire.begin(sda, scl);

  Wire.setClock(speed);                      //experimental! STM32 I2C bus speed ???kHz..400kHz, library default 100000Hz
